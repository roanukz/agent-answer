...
export const slowStart: Rule = {
  ...
  run(doc) {
    ...
    for (const section of doc.sections) {
      ...
      if (contentWords(section.heading).some((w) => openerWords.has(w))) {
        continue
      }
      findings.push({
        ruleId: 'slow-start',
        ...
        severity: 'minor',
        span: first.span,
        message: `This section opens with a ${first.wordCount}-word paragraph that never restates "${truncate(section.heading, 60)}" up front and gives no instruction.`,
        whyItMatters:
